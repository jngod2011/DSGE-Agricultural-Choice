# -*- coding: utf-8 -*-
"""
Created on Tue Oct 30 12:28:59 2018

@author: rodri
"""

# =============================================================================
# 2 periods model: No Insurance case
# =============================================================================
import numpy as np
...
plt.show()
fig.savefig('C:/Users/rodri/OneDrive/Documentos/IDEA/Phd tesi/figures/2period_wealth.png')
...
plt.show()
fig.savefig('C:/Users/rodri/OneDrive/Documentos/IDEA/Phd tesi/figures/2period_inputprices.png')
...
#problem with nans
b_grid= np.linspace(-100,0,50)   #For prices above 2.29 dont have solution
for b in b_grid:
    def U(x):
    # *args pi_θ=pi_θ , pi_ε = pi_ε, θ = θ ,ε = ε, u=u, r=r, β = β
        a = x[0]
        ...
        #return - (u(max(y_0 +a_0 -p*(m1 + m2) -a, 0.00001)) +β*u(max(A*m1**α +B*m2**γ +(1+r)*a,0.00001))*pi_θ[1]*pi_ε[1] +β*u(max(B*m2**γ +(1+r)*a,0))*pi_θ[0]*pi_ε[1] + β*u(max((1+r)*a,0.00001))*pi_θ[0]*pi_ε[0] + β*u(max((1+r)*a,0.00001))*pi_θ[1]*pi_ε[0])
        return - (u(max(y_0 +a_0 -p*(m1 + m2) -a , 0.0001)) +β*u(A*m1**α +B*m2**γ +(1+r)*a)*pi_θ[1]*pi_ε[1] +β*u(B*m2**γ  +(1+r)*a)*pi_θ[0]*pi_ε[1] + β*u((1+r)*a )*pi_θ[0]*pi_ε[0] + β*u((1+r)*a)*pi_θ[1]*pi_ε[0])

    x0 = np.array([2,2,2])
    bounds = ((b,100), (0, 100) , (0, 100)) 
    res = minimize(U, x0, method='SLSQP' , bounds=bounds)
    policy_a.append(res.x[0])
    policy_m1.append(res.x[1])
    policy_m2.append(res.x[2])
...
plt.show()
fig.savefig('C:/Users/rodri/OneDrive/Documentos/IDEA/Phd tesi/figures/2period_borrowcons_.png')
...
plt.show()
fig.savefig('C:/Users/rodri/OneDrive/Documentos/IDEA/Phd tesi/figures/2period_interestrate_.png')
...
fig.savefig('C:/Users/rodri/OneDrive/Documentos/IDEA/Phd tesi/figures/2period_riskaversion_rich_.png')
