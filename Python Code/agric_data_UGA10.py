...
data10 = data10[['hh', 'inflation', 'ctotal', 'region']]
data10.columns = ['HHID', 'inflation', 'ctotal', 'region' ]


data.rename(columns={'pltid':'plotID'}, inplace=True)
data = pd.merge(data, data10, on='HHID', how='left')
...
variables = ['k', 'm', 'l', 'A', 'y', 'y_over_A']
for var in variables:
    data['ln'+var] = data['ln'+var] = np.log(data[var].dropna()+np.abs(np.min(data[var]))).replace(-np.inf, np.nan)
    

data.to_csv('C:/Users/rodri/OneDrive/Documentos/IDEA/Phd tesi/data/agric_data10.csv', index=False)
